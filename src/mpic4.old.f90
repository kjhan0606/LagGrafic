...
subroutine mpic4(idim,irand,iseed,itide,m1s,m2s,m3s,m1off, &
     &    m2off,m3off,hanning,filename,astart,pk,dx,xoff,f,g,fm, &
  ...
  do j=0,npow
     ...
     if (idim.gt.0 .and. idim .lt. 4 .and.j.gt.0) then
        tsav(j)=tsav(j)/(ak*ak)
     end if
  end do
...
!             if (k3+local_z_start.eq.n32+1) ctemp=0.0
  ...
  if(idim .gt. 0 .and. idim .lt.4) then
     call twolpt(idim,plan,iplan,f,total_local_size,local_nz,local_z_start,dx,g)
  endif
