...
function p(ak,a)
  ...
  parameter (nkmax=1000,nt=11,mkmax=8196)
  ...
  integer ncamblines,ired
  real  kcamb(mkmax),pktcamb(mkmax,2),pkbcamb(mkmax,2),pkccamb(mkmax,2)
  real  ft(mkmax),fb(mkmax),fc(mkmax)
  double precision dlnkh, minkh
  ...
  elseif (icase .eq.5) then
     ...
     do i = 1, ncamblines-1
        if(kcamb(i) .le. akh .and. kcamb(i+1) .ge. akh) then
           p = (pktcamb(i+1,ired)-pktcamb(i,ired))/(kcamb(i+1)-kcamb(i))*(akh-kcamb(i)) + pktcamb(i,ired)
!          p = p*pnorm
           return
        endif
     enddo
...
function ptot(ak,a)
  ...
  elseif (icase .eq.5) then
     ...
     do i = 1, ncamblines-1
        if(kcamb(i) .le. akh .and. kcamb(i+1) .ge. akh) then
           ptot = (pktcamb(i+1,ired)-pktcamb(i,ired))/(kcamb(i+1)-kcamb(i))*(akh-kcamb(i)) + pktcamb(i,ired)
!          pcdm = pcdm*pnorm
           return
        endif
     enddo
...
function pcdm(ak,a)
  ...
  elseif (icase .eq.5) then
     ...
     do i = 1, ncamblines-1
        if(kcamb(i) .le. akh .and. kcamb(i+1) .ge. akh) then
           pcdm = (pkccamb(i+1,ired)-pkccamb(i,ired))/(kcamb(i+1)-kcamb(i))*(akh-kcamb(i)) + pkccamb(i,ired)
!          pcdm = pcdm*pnorm
           return
        endif
     enddo
...
function pbar(ak,a)
  ...
  elseif (icase .eq.5) then
     ...
     do i = 1, ncamblines-1
        if(kcamb(i) .le. akh .and. kcamb(i+1) .ge. akh) then
           pbar = (pkbcamb(i+1,ired)-pkbcamb(i,ired))/(kcamb(i+1)-kcamb(i))*(akh-kcamb(i)) + pkbcamb(i,ired)
!          pbar = pbar*pnorm
           return
        endif
     enddo
...
subroutine pini(anorml)
  ...
  if (myid==0) then
     ...
     write(*,*) '    5 for T(k) from CAMB test files (zini)    '
     write(*,*) 'Enter case (1,2,3,4 or 5) now:'
     read(*,*) icase
     print*, icase
  endif
  ...
  if (icase.eq.1) then
     ...
     if (h0.eq.0) then
        ...
        if (myid==0) then
           ...
           write(*,*) 'Enter w0, wa, cs2 for DE model'
           read(*,*) w0,wa,cs2
           print *, w0,wa,cs2
        endif
        ...
        call mpi_bcast(cs2,1,mpi_real,0,mpi_comm_world,ierr)
  ...
  if (myid==0) then
     ...
     if (icase.eq.1.or.icase.eq.2.or.icase.eq.4.or.icase.eq.5) then
        write(*,*) '  Qrms-ps/micro-K (if > 0) or -sigma_8 (if < 0) = ? or 0 for no renormalization'
     else
  ...
  if (icase.eq.2.or.icase.eq.4 .or. icase.eq.5) then
     ...
     if(icase.eq.5) then
        if(myid==0) then
          write(*,*) 'Enter camb.redi.txt  filename'
          read(*,'(a)') filename
          open(11,file=filename,status='old')
          do i = 1, 3
             read(11,*)
          enddo
          read(11,'("npoints= ", i5," minkh=", g14.7, " dlnkh= ", g14.7)') ncamblines, minkh, dlnkh
          print *,'ncamblines= ', ncamblines,minkh,dlnkh
          read(11,*)
          do i = 1, ncamblines
             read(11,*) kcamb(i),pktcamb(i,1),pkccamb(i,1),pkbcamb(i,1),&
                ft(i), fc(i),fb(i),dt,dc,db

             ! kcamb(i):= k/h
             pktcamb(i,1) = pktcamb(i,1)/(h0/100.)**3
             pkbcamb(i,1) = pkbcamb(i,1)/(h0/100.)**3
             pkccamb(i,1) = pkccamb(i,1)/(h0/100.)**3
             ! now P(k) is in unit of Mpc^3


             pktcamb(i,1) = pktcamb(i,1)/twopi/twopi/twopi
             pkbcamb(i,1) = pkbcamb(i,1)/twopi/twopi/twopi
             pkccamb(i,1) = pkccamb(i,1)/twopi/twopi/twopi
             pktcamb(i,2) = pktcamb(i,1)/dt
             pkbcamb(i,2) = pkbcamb(i,1)/db
             pkccamb(i,2) = pkccamb(i,1)/dc
          enddo
          close(11)

!         write(*,*) 'Enter camb.red0.txt  filename'
!         read(*,'(a)') filename0
!         open(11,file=filename0,status='old')
!         do i = 1, 4
!            read(11,*)
!         enddo
!         read(11,*) ncamblines
!         print *,'ncamblines= ', ncamblines
!         read(11,*)
!         do i = 1, ncamblines
!            read(11,*) kcamb(i),pktcamb(i,2),pkccamb(i,2),pkbcamb(i,2)
!            pktcamb(i,2) = pktcamb(i,2)/twopi/twopi/twopi
!            pkbcamb(i,2) = pkbcamb(i,2)/twopi/twopi/twopi
!            pkccamb(i,2) = pkccamb(i,2)/twopi/twopi/twopi
!         enddo
!         close(11)
        endif
        call mpi_bcast(ncamblines,1,mpi_integer,0,mpi_comm_world,ierr)
        call mpi_bcast(minkh,1,mpi_real8,0,mpi_comm_world,ierr)
        call mpi_bcast(dlnkh,1,mpi_real8,0,mpi_comm_world,ierr)
        call mpi_bcast(kcamb(1),mkmax,mpi_real,0,mpi_comm_world,ierr)
        call mpi_bcast(pktcamb(1,1),mkmax,mpi_real,0,mpi_comm_world,ierr)
        call mpi_bcast(pkbcamb(1,1),mkmax,mpi_real,0,mpi_comm_world,ierr)
        call mpi_bcast(pkccamb(1,1),mkmax,mpi_real,0,mpi_comm_world,ierr)
        call mpi_bcast(pktcamb(1,2),mkmax,mpi_real,0,mpi_comm_world,ierr)
        call mpi_bcast(pkbcamb(1,2),mkmax,mpi_real,0,mpi_comm_world,ierr)
        call mpi_bcast(pkccamb(1,2),mkmax,mpi_real,0,mpi_comm_world,ierr)
        call mpi_bcast(ft(1),mkmax,mpi_real,0,mpi_comm_world,ierr)
        call mpi_bcast(fc(1),mkmax,mpi_real,0,mpi_comm_world,ierr)
        call mpi_bcast(fb(1),mkmax,mpi_real,0,mpi_comm_world,ierr)
     endif
  else
     !**************************************
...
! akmax=h0/8.0
  ...
  if (anorml.gt.0.0) then
     ...
     if (myid==0) write(*,*) 'Qrms-ps/micro-K=',qrmsps,sig0
  else
     pnorm = 1.0
  end if
  ...
  if (myid==0) then
     if(icase .eq.4) then
     open(11,file='power.eh.dat',form='formatted',status='unknown')
     else if(icase .eq.5) then
     open(11,file='power.camb.dat',form='formatted',status='unknown')
     else 
     open(11,file='power.dat',form='formatted',status='unknown')
     endif
     rewind 11
     write(11,*) an,anorml,h0/100.
     do i=1,nkplot
        ak0=ak1*exp((i-1)*dlkp)
! I multiply a factor (2pi)**3 for the ordinary definition of power
        write(11,*) ak0/(h0/100.0),p(ak0,1.0)*(h0/100.0)**3*twopi3, &
             &                fourpi*ak0*ak0*ak0*p(ak0,1.0)*twopi3
!       write(11,*) ak0,p(ak0,1.0),fourpi*ak0*ak0*ak0*p(ak0,1.0)
     end do
     close(11)
  endif
