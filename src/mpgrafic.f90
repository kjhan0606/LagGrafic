...
#define  rfftwnd_f77_mpi_local_sizes rfftwnd_f77_mpi_local_sizes_
...
#ifdef SING
  real(sp), dimension(:), allocatable :: f,g,phi1,phi2
#else
  real(dp), dimension(:), allocatable :: f,g,phi1,phi2
#endif
  ...
  real astart,omegam,omegav,omegavz,h0,dladt,fomega,vfact,vfact2,pbar,pcdm, ptot,zp1,omei,redi
  ...
  character(len=128) :: outputfile,outputfile2
  real w0,wa,cs2
  common /cosmoparms/ omegam,omegav,h0,w0,wa,cs2
  common /cosmoparms2/ omegab,omegahdm,degen_hdm
  common /dsig/ asig,dxr
  external pbar,pcdm, ptot,dsigma,dsigmadispl,rombin,dplus,adp
  real anorml
  integer itotflag, icdmflag, ibarflag ! flag to initialize and to calculate phi1, phi2
  integer kk
...
!!$  sigma=MAX(sigma,sigmadispl/dxr)
  ...
  ! modified by Juhan for the input astart
  if(.false.)then
  dpls=sigstart/sigma*dplus(1.0,omegam,omegav)
  astart=adp(dpls,omegam,omegav)
  else
     if(myid == 0)then
     print *,'input initial redshift'
     read(*,*) redi
     astart = 1./(redi+1)
     endif
     call mpi_bcast(astart,1,mpi_real,0,mpi_comm_world,ierr)
     dpls = dplus(astart,omegam,omegav)/dplus(1.0,omegam,omegav)
     sigstart = sigma*dpls
     if(myid==0) print*,'astart= ', astart,'sigstart=', sigstart
  endif
...
!!!!!!!! original grafic1 starting conditions....
  ...
  omei = omegam*zp1**3/(omegam*zp1**3+omegavz+(1-omegam-omegav)*zp1**2)

  if(anorml .eq. 0.0) then
  else
  vfact=fomega(astart,omegam,omegav) &
       &       *h0*dladt(astart,omegam,omegav)/astart
  ! inserted for 2LPT by kjhan 05/24/2021
  vfact2 = -3./7.*2*omei**(4./7.)*h0*dladt(astart,omegam,omegav)/astart
  endif



  if (myid==0) write(*,*)'vfact=',vfact,'vfact2=',vfact2
  !
...
! dump white noise data to f()
     ...
     ! if not, no change in the amplitude
       pnorm = 1.0
     endif
...
! endif
  ...
  allocate(phi1(total_local_size),stat=status)
  allocate(phi2(total_local_size),stat=status)
!
  !  Outer loop over components to be refined.
  !
  do icomp=0,22
     !  0: baryon density.
     !  1,2: inner,outer baryon x-vector. idim=1
     !  3,4: inner,outer baryon y-vector. idim=2
     !  5,6: inner,outer baryon z-vector. idim=3
     !  7,8: inner,outer CDM x-vector.idim=4
     !  9,10: inner,outer CDM y-vector.idim=5
     !  11,12: inner,outer CDM z-vector.idim=6
     !  14: cdm density  idim=7
     !  15,16: inner,outer total x-vector idim=8
     !  17,18: inner,outer total y-vector idim=9
     !  19,20: inner,outer total z-vector idim=10
     !  22: total density idim = 11
     !
     ...
     if (icomp.gt.0.and.icomp.le.6) then
        xoff=0.5*dx*offvelb
     else if (icomp.gt.6 .and. icomp .le. 12) then
        xoff=0.5*dx*offvelc
     else
        xoff=0.0
     end if
     ...
     !
     !  Generate random fields.  idim=0,1,2,3 for density, displacement, and vel. 
     itide=0
     ...
     allocate(g(total_local_size),stat=status)
     if (status /= 0) then
     ...
     headc%omegav=omegav
     headc%h0=h0
     ! now fetch phi(1) and phi(2) from the white noise and input power
     if(icomp .ge. 15 .and. itotflag.eq.0) then
        ! total: matter except neutrino
        call getPhi1Phi2(irand,iseed,itide,m1t,m2t,m3t,m1offt,m2offt, &
             &     m3offt,hanning,filename,astart,ptot,dx,xoff,phi1,phi2,fm, &
             &     plan,iplan,local_nz,local_z_start,total_local_size, &
             &     headt,headc,small_kfile_name,ipad, anorml)
        itotflag = 1
     else if(icomp .ge. 7 .and. icdmflag.eq.0) then
        ! cdm
        call getPhi1Phi2(irand,iseed,itide,m1t,m2t,m3t,m1offt,m2offt, &
             &     m3offt,hanning,filename,astart,pcdm,dx,xoff,phi1,phi2,fm, &
             &     plan,iplan,local_nz,local_z_start,total_local_size, &
             &     headt,headc,small_kfile_name,ipad, anorml)
        icdmflag = 1
     else if(icomp .lt. 7 .and. ibarflag.eq.0) then
        ! baryon
        call getPhi1Phi2(irand,iseed,itide,m1t,m2t,m3t,m1offt,m2offt, &
             &     m3offt,hanning,filename,astart,pbar,dx,xoff,phi1,phi2,fm, &
             &     plan,iplan,local_nz,local_z_start,total_local_size, &
             &     headt,headc,small_kfile_name,ipad, anorml)
        ibarflag = 1
     endif
     ...
     if (idim.le.3) then
        !-- This is for the baryon matter. Outputs are f and g
        call mpic4(icomp,idim,irand,iseed,itide,m1t,m2t,m3t,m1offt,m2offt, &
             &     m3offt,hanning,filename,astart,'baryon',dx,xoff,f,g,fm, &
             &     plan,iplan,local_nz,local_z_start,total_local_size, &
             &     headt,headc,small_kfile_name,ipad, anorml,phi1,phi2,omei)
     else if(idim.le.7) then
        !-- This is for the cold dark matter. Outputs are f and g
        idim=idim-3
        call mpic4(icomp,idim,irand,iseed,itide,m1t,m2t,m3t,m1offt,m2offt, &
             &     m3offt,hanning,filename,astart,'cdm',dx,xoff,f,g,fm, &
             &     plan,iplan,local_nz,local_z_start,total_local_size, &
             &     headt,headc,small_kfile_name,ipad, anorml,phi1,phi2,omei)
     else 
        !-- This is for the total matter. Outputs are f and g
        idim=idim-7
        call mpic4(icomp,idim,irand,iseed,itide,m1t,m2t,m3t,m1offt,m2offt, &
             &     m3offt,hanning,filename,astart,'total',dx,xoff,f,g,fm, &
             &     plan,iplan,local_nz,local_z_start,total_local_size, &
             &     headt,headc,small_kfile_name,ipad, anorml,phi1,phi2,omei)
     end if
     ...
     !
     if (.true.) then
        print *, icomp, idim, myid, g(100),f(100)
        !  Output files.
        if (icomp.eq.0) then
           outputfile='ic_deltab'
        else if (icomp.eq.2) then
           outputfile='ic_velbx'
           outputfile2='ic_posbx'
        else if (icomp.eq.4) then
           outputfile='ic_velby'
           outputfile2='ic_posby'
        else if (icomp.eq.6) then
           outputfile='ic_velbz'
           outputfile2='ic_posbz'
        else if (icomp.eq.8) then
           outputfile ='ic_velcx'
           outputfile2='ic_poscx'
        else if (icomp.eq.10) then
           outputfile ='ic_velcy'
           outputfile2='ic_poscy'
        else if (icomp.eq.12) then
           outputfile ='ic_velcz'
           outputfile2='ic_poscz'
        else if (icomp.eq.14) then
           outputfile='ic_deltac'
        else if (icomp.eq.16) then
           outputfile ='ic_veltx'
           outputfile2='ic_postx'
        else if (icomp.eq.18) then
           outputfile ='ic_velty'
           outputfile2='ic_posty'
        else if (icomp.eq.20) then
           outputfile ='ic_veltz'
           outputfile2='ic_postz'
        else if (icomp.eq.22) then
           outputfile='ic_deltat'
        end if
        if (myid==0) then
           headt%nx=np1
           headt%ny=np2
           ...
           if(idim.gt.0 .and. idim .lt. 4 ) then
              call grafic_write_header(outputfile2,headt,headc)
           endif
        endif
        call mpi_barrier(mpi_comm_world,ierr)
        call grafic_write(f,local_nz,local_z_start,ny,nx,outputfile)
        if(idim.gt.0 .and. idim .lt. 4) then
           call grafic_write(g,local_nz,local_z_start,ny,nx,outputfile2)
        endif
