...
! -- inserted by kjhan because pcorr & f_growth should not be scaled with k^2
    Arr(Transfer_poisson_factor) = Arr(Transfer_poisson_factor)*EV%k2_buf
    Arr(Transfer_clxcdot) = Arr(Transfer_clxcdot)*EV%k2_buf
    Arr(Transfer_clxbdot) = Arr(Transfer_clxbdot)*EV%k2_buf
    Arr(Transfer_clxnonudot) = Arr(Transfer_clxnonudot)*EV%k2_buf
! -- inserted by kjhan because pcorr & f_growth should not be scaled with k^2
    ...
    if (associated(EV%OutputTransfer) .or. associated(EV%OutputSources)) then
        ...
        if (associated(EV%OutputTransfer)) then
            ...
            EV%OutputTransfer(Transfer_nonu) = (grhob_t*clxb+grhoc_t*clxc)/(grhob_t + grhoc_t) ! without neutrinos
...
!------------------
! inserted by kjhan to include the relativistic effect on the Poisson equation
#if defined(_CDM_PK_)
! This is for the cdm P(k). 
            EV%OutputTransfer(Transfer_poisson_factor) = (dgrho + 3*dgq*adotoa/k)/EV%Kf(1)/(grho_matter*clxc)
#else
! This is for the total matter P(k).
            EV%OutputTransfer(Transfer_poisson_factor) = (dgrho + 3*dgq*adotoa/k)/EV%Kf(1)/dgrho_matter
#endif
!           EV%OutputTransfer(Transfer_clxcdot) = clxcdot/clxc
!           EV%OutputTransfer(Transfer_clxbdot) = clxbdot/clxb
            EV%OutputTransfer(Transfer_clxcdot) = clxcdot/(clxc*adotoa)
            EV%OutputTransfer(Transfer_clxbdot) = clxbdot/(clxb*adotoa)
            EV%OutputTransfer(Transfer_clxnonudot) = (grhob_t*clxbdot/clxb+grhoc_t*clxcdot/clxc)/(grhob_t + grhoc_t)/adotoa
!------------------
