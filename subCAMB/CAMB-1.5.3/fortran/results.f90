...
! inserted by kjhan @26092024
    function music_open_file_header(filename, Col1, Columns, n) result(unit)
    character(LEN=*), intent(in) :: filename
    character(LEN=*), intent(in) :: col1
    character(LEN=name_tag_len), intent(in) :: Columns(:)
    integer, intent(in), optional :: n
    ...
    open(newunit=unit,file=filename,form='formatted',status='replace')
    if (output_file_headers) then
        write(unit,'("#",1A'//Trim(IntToStr(nn-1))//'," ",*(A15))') Col1, Columns(1:12)
    end if

    end function music_open_file_header
...
!-----------------
! inserted by kjhan
        Transfer_poisson_factor = 14, &! Poisson correction factor
        Transfer_clxnonudot = 15, &! dD/dtau for matter exclusing neutrino
        Transfer_clxcdot = 16, &! dD/dtau for cdm
        Transfer_clxbdot = 17 ! dD/dtau for baryon
!-----------------
    !Sources
...
!-- inserted by kjhan (22/05/24)
! This is for wxtract the growth rate stored in State
    subroutine Transfer_Getgrowthrate(State, MTrans, outpower, itf, minkh, dlnkh, npoints, var1)
    class(CAMBdata) :: state
    Type(MatterTransferData), intent(in) :: MTrans
    integer, intent(in) :: itf, npoints
    ...
    minkhd = minkh; dlnkhd = dlnkh
    call Transfer_GetgrowthrateD(State, MTrans, outpowerd, itf, minkhd, dlnkhd, npoints,var1)
    outpower(1:npoints) = outpowerd(1:npoints)

    end subroutine Transfer_Getgrowthrate

    subroutine Transfer_GetgrowthrateD(State, MTrans, outpower, itf_PK, minkh, dlnkh, npoints, var1)
    !Allows for non-smooth priordial spectra
    !if CP%Nonlinear/ = NonLinear_none includes non-linear evolution
    !Get total matter power spectrum at logarithmically equal intervals dlnkh of k/h starting at minkh
    !in units of (h Mpc^{-1})^3.
    ...
    integer ik, llo,il,lhi,lastix
    real(dl) growthRate(MTrans%num_q_trans), kh, kvals(MTrans%num_q_trans), ddmat(MTrans%num_q_trans)
    real(dl) atransfer,xi, a0, b0, ho, logmink,k, h
    integer itf
    integer :: s1,s2, sign
    logical log_interp
...
!   end if
    ...
    do ik=1,MTrans%num_q_trans
        ...
        k = kh*h
        kvals(ik) = log(kh)
        growthRate(ik)=MTrans%TransferData(s1,ik,itf)
        !Put in power spectrum later: transfer functions should be smooth, initial power may not be
    end do
    sign = 1
    log_interp = .true.
    if (any(growthRate <= 0)) then
        if (all(growthRate < 0)) then
            sign = -1
        else
            log_interp = .false.
        endif
    endif
    if (log_interp) growthRate = log(sign*growthRate)
    call spline_def(kvals,growthRate,MTrans%num_q_trans, ddmat)
    ...
    do il=1, npoints
        ...
        outpower(il) = a0*growthRate(llo)+ b0*growthRate(lhi)+((a0**3-a0)* ddmat(llo) &
            +(b0**3-b0)*ddmat(lhi))*ho**2/6
    end do
...
!       do il = 1, npoints
!           k = exp(logmink + dlnkh*(il-1))*h
!           outpower(il) = outpower(il) * sqrt(InitPower%ScalarPower(k))
!           if (global_error_flag /= 0) exit
!       end do
!   end associate

    end subroutine Transfer_GetgrowthrateD
!-- inserted by kjhan 22/05/24
...
! inserted by kjhan @25092024
    subroutine Music_Transfer_SaveToFiles(MTrans,State,FileNames)
    use constants
    Type(MatterTransferData), intent(in) :: MTrans
    class(CAMBdata) :: State
    integer i,ik
    ...
    do i_PK=1, State%CP%Transfer%PK_num_redshifts
        if (FileNames(i_PK) /= '') then
            ...
            if (State%CP%do21cm) then
                ...
                unit = music_open_file_header(FileNames(i_PK), 'k/h', transfer_name_tags, 14)
            end if
            do ik=1,MTrans%num_q_trans
                if (MTrans%TransferData(Transfer_kh,ik,i)/=0) then
                    write(unit,'(*(E15.6))') MTrans%TransferData(Transfer_kh:13,ik,i)
                end if
            end do
            close(unit)
        end if
    end do
    end subroutine Music_Transfer_SaveToFiles
! inserted by kjhan @25092024
